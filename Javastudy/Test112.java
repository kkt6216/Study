...
//	  하위 클래스의 생성자에서 상위 클래스의 생성자를 호출 할때에는
...
//	  이때, 상위 클래스의 생성자는
//	  인수가 업는 생성자(default 생성자 형태)가 호출된다.

//	  상위 클래스 및 하위 클래스(즉, 상속관계에 있는 클래스)를 설계하는 과정에서 
//	  상위 클래스의 생성자를 정의하지(작성하지) 않거나
//	  인수가 없는 생성자만 정의한(작성한) 경우
//	  명시적을 하위 클래스에서 상위 클래스의 생성자를 호출하지 않아도 
//	  아무런 문제가 발생하지 않지만
...
//	  따라서, Bclass 생성자의 선두에
//	  다음처럼 명시적 상위 클래스의 생성자 호출 구문을 작성해야 한다.
