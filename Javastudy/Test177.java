...
○ 객체 직렬화의 장점
   ...
   RMI 는 원래 객체 통신을 지원해야 하기 떄문에
   객체가 그대로(특정 상태를 유지한 채로) 이동할 수 있어야 한다.
   따라서 이를 지원하기 위해서는 객체 직렬화가 필수적이었다.

   또한,  Bean 은 설계 시 상태에 대한 정보를 저장할 떄
   이 객체 직렬화를 사용하면 편게 객체 상태를저장할 수 있다.
   ...
   ※ Bean(빈, 자바 빈)
	  ...
	  클라이언트에게 빈이라는 프로그램 커모넌트를 분배하는 방식으로 처리

 ○ Serializable 인터페이스

    객체 직렬화를 하기 위해 먼저 객체 직렬화가 가능하도록
	java.io.Serializable 인터페이스를 구현해 주어야한다.
	이 인터페이스는 객체 직렬화가 제공되어야 한다는 사실을
	JVM 에 알려주는 역할을 수행한다.
	또한,  Serializble 인터페이스는
...
public class Test177
{
	public static void main(String[] args) throws Exception
	{
		...
		if (f0.exists())
		{
			...
			// fis 파일 입력스트림으로 부터 객체 입력 스트림(ois, ObjectInputStream)을 얻어내어
			ObjectInputStream ois = new ObjectInputStream(fis);

			// 객체 입력 스트림(ois, ObjectInputStream)으로 읽어들인 객체(Object)를
			// 캐스팅(Hashtable)하여 h2라는 자료구조에 담아내기
